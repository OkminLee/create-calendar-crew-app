...
### 플랫폼 선택
- **웹앱 (PWA)**: 기본값, 모바일/데스크톱 모두 지원
- **iOS 앱**: SwiftUI 기반 (추후 지원 예정)
- **Android 앱**: Kotlin 기반 (추후 지원 예정)
...
### 백엔드 설정 (자동 추천)
선택한 기능에 따라 적합한 백엔드를 추천합니다:
- **기본 기능만**: 백엔드 불필요 (Firebase만 사용)
- **푸시 알림**: Cloudflare Workers 추천
- **Slack 연동**: Cloudflare Workers 추천
...
[4단계: 백엔드 추천]
Claude: "푸시 알림 기능을 선택하셨네요. Cloudflare Workers를 추천드립니다.
       무료 티어로 충분하고, wrangler로 간편하게 배포할 수 있습니다."
→ 추천 수락
...
질문: 어떤 플랫폼으로 생성할까요?
옵션:
- 웹앱 (PWA) - 추천 (Recommended)
- iOS 앱 (추후 지원)
- Android 앱 (추후 지원)
```
> 웹앱(PWA)이 기본값입니다. iOS/Android 선택 시 "현재 웹앱만 지원됩니다"라고 안내하고 웹앱으로 진행합니다.
...
**4단계: 백엔드 추천** (알림/Slack 선택 시)
선택한 기능에 따라 백엔드를 자동 추천합니다:

```
추천 로직:
- 기본 기능만 (참여 신청, 댓글): 백엔드 불필요 → Firebase만 사용
- 푸시 알림 선택: Cloudflare Workers 추천 (무료 티어, 간편 배포)
- Slack 연동 선택: Cloudflare Workers 추천 (Webhook 처리)
- 둘 다 선택: Cloudflare Workers 추천

추천 메시지 예시:
"선택하신 기능(푸시 알림, Slack 연동)을 위해 Cloudflare Workers를 추천드립니다.
무료 티어로 충분하고, 배포가 간편합니다. 다른 옵션을 원하시면 알려주세요."
